...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function positionTooltip(target, text) {
      ...
      // Lesen des erzwungenen Positions-Attributs (top|bottom)
      const forced = target.getAttribute('data-tooltip-position');
  
      let top, pos;
      // Forced-Position beachten
      if (forced === 'top') {
        top = targetRect.top - contentRect.height - 16;
        pos = 'top';
      } else if (forced === 'bottom') {
        top = targetRect.bottom + 8;
        pos = 'bottom';
      } else {
        // Standard: prüfen, ob genug Platz oben ist
        top = targetRect.top - contentRect.height - 16;
        pos = 'top';
        if (top < 8) {
          top = targetRect.bottom + 8;
